...
const EXPENSE_KEYS: Exclude<
  keyof OnboardingData['expenses'],
  'educationFrequency'
>[] = [
  'food',
  'groceries',
...
export function computeFinance(data: OnboardingData): ComputedFinance {
  ...
  // MONTHLY INCOME
  // ============================================================
  // Salary field in onboarding is MONTHLY salary.
  // So we must NOT divide it by 12.
  const monthlyIncome =
    data.career.salary + data.career.monthlyBonus;

  // ============================================================
  // MONTHLY EXPENSES
  // ============================================================
  // Education can be monthly, semester-wise, or yearly.
  // Convert it into a monthly equivalent before adding it.
 const monthlyExpenses = EXPENSE_KEYS.reduce(
  (sum, k) => {
    const amount = Number(data.expenses[k] || 0);

    if (k === 'education') {
      const frequency =
        data.expenses.educationFrequency ?? 'monthly';
  ...
  const investmentValue = INVESTMENT_KEYS.reduce(
    (sum, k) => sum + (data.savings[k] || 0),
    0
  );

  const totalAssets = SAVINGS_KEYS.reduce(
    (sum, k) => sum + (data.savings[k] || 0),
    0
  );

  const netWorth =
    totalAssets - (data.lifestyle.loans || 0);
  ...
  const debtToIncomeRatio =
    monthlyIncome > 0
      ? ((data.lifestyle.emis || 0) / monthlyIncome) * 100
      : 0;

  const fiScore = computeFIScore(
    monthlySavings,
    monthlyExpenses,
    investmentValue,
    data
  ...
  const cashFlow = [
    ...
    {
      name: 'Investments',
      value: Math.round(monthlySavings * 0.6),
    },
    {
      name: 'Remaining',
      value: Math.round(
        Math.max(0, monthlySavings * 0.4)
      ),
    },
  ];
  ...
  const aiInsights = generateInsights(
    ...
    personalInflationIndex,
    monthlyExpenses > 0
      ? data.savings.emergencyFund / monthlyExpenses
      : 0
  );
  ...
  const emergencyFund = {
    ...
    monthsCovered:
      Math.round(monthsCovered * 10) / 10,
    pct:
      recommendedEmergency > 0
        ? Math.min(
            100,
            (data.savings.emergencyFund /
              recommendedEmergency) *
  ...
  return {
    ...
    savingsRate:
      Math.round(savingsRate * 10) / 10,

    expenseRatio:
      Math.round(expenseRatio * 10) / 10,
...
function computeInflationIndex(
  data: OnboardingData,
  monthlyExpenses: number
): {
  personalInflationIndex: number;
  ...
  const cityKey =
    data.personal.city?.toLowerCase() ?? '';

  const cityMultiplier =
    CITY_INFLATION_ADJUSTMENTS[
      cityKey
    ] ?? 1.0;

  const familyMultiplier =
    1 +
    (data.family.familyMembers - 1) *
      0.03;
  ...
  for (const k of EXPENSE_KEYS) {
    ...
    const w =
      CATEGORY_INFLATION_WEIGHTS[k] ??
      0.02;

    const shareOfMonthly =
      monthlyExpenses > 0
        ? spend / monthlyExpenses
        : 0;

    const categoryMultiplier =
      spend > 0
        ? 1 + shareOfMonthly * 0.5
        : 0.5;
...
function computeHealthScore(
  ...
  const debtRatio =
    totalAssets > 0
      ? (data.lifestyle.loans || 0) /
        totalAssets
      : 0;

  const hasInsurance =
    (data.lifestyle.insurance || 0) >
    0;
...
function generateInsights(
  ...
  if (foodPct > 20) {
    insights.push({
      title: `You spend ${Math.round(
        foodPct
      )}% more on food`,

      desc:
        'Consider meal planning and home cooking to save up to ₹3,000/month.',
  ...
  if (savings > 0) {
    insights.push({
      ...
      desc:
        'Auto-invest this into a SIP to build long-term wealth.',
  ...
  if (
    ...
    insights.push({
      ...
      desc: `You spend ₹${data.expenses.subscriptions.toLocaleString(
        'en-IN'
      )}/mo on subscriptions. Audit and cancel unused ones.`,
  ...
  if (emergencyMonths < 3) {
    insights.push({
      title:
        'Emergency fund below recommended',

      desc: `You have ${emergencyMonths.toFixed(
        1
      )} months covered. Aim for 6 months of expenses.`,
  ...
  if (inflation > NATIONAL_AVG) {
    insights.push({
      title:
        'Your personal inflation is above average',
  ...
  if (
    ...
    insights.push({
      ...
      desc:
        'Increase SIP by ₹2,000/month to beat inflation and build corpus.',
  ...
  if (
    ...
    insights.push({
      title:
        'Shopping exceeds 10% of income',

      desc:
        'Set a shopping budget to avoid lifestyle creep.',
  ...
  if (
    ...
    insights.push({
      ...
      desc:
        'Prioritize paying down loans before increasing investments.',
...
function generateAllocation(
  data: OnboardingData
): ComputedFinance['investmentAllocation'] {
  const age =
  ...
  return [
    {
      ...
      desc:
        'Low-cost broad market exposure for long-term growth.',
      expectedReturn:
...

    {
      ...
      desc:
        'Stable returns to cushion market volatility.',
      expectedReturn:
        ASSET_EXPECTED_RETURNS[
          'Debt Funds'
        ],
...

    {
      ...
      pct: 15,
      desc:
        'Tax-advantaged long-term retirement savings.',
      expectedReturn:
        ASSET_EXPECTED_RETURNS.PPF,
    },

    {
      ...
      pct: 10,
      desc:
        'Liquid funds for 6 months of expenses.',
      expectedReturn:
        ASSET_EXPECTED_RETURNS[
          'Emergency Cash'
        ],
...
function optimizeExpenses(
...
): ComputedFinance['expenseOptimization'] {
  ...
  if (
    ...
    items.push({
      ...
      lifestyleImpact:
        'Cancel 2-3 unused streaming/app subscriptions.',
  ...
  if (
    ...
    items.push({
      ...
      lifestyleImpact:
        'Book flights early, use reward points.',
  ...
  if (
    ...
    items.push({
      ...
      lifestyleImpact:
        'Swap paid outings for free community events.',
